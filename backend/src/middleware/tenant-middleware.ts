...
// Uses simple middleware approach
...
export function tenantMiddleware(prisma: PrismaClient) {
  ...
  prisma.$use(async (params, next) => {
    ...
    // Get tenant context from global storage (set by tenantContext middleware)
    const tenantId = (globalThis as any).__TENANT_CONTEXT__?.tenantId;

    if (tenantId) {
      // Add tenant_id to where clauses for multi-tenant models
      const model = params.model;
      const multiTenantModels = [
        'User', 'Asset', 'AssetEvent', 'AuditSession', 'AuditSessionItem', 'AuditDiscrepancy',
        ...
        if (['create', 'createMany', 'upsert'].includes(params.action)) {
          if (params.args.data) {
            if (Array.isArray(params.args.data)) {
              ...
              params.args.data = {
                ...params.args.data,
                tenant_id: params.args.data.tenant_id || tenantId,
              };
            }
          }
        }
