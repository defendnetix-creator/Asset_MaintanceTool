...
export async function auditRoutes(app: FastifyInstance) {
    ...
    schema: {
      ...
      if (request.body.scope_id) {
        ...
        switch (request.body.scope_type) {
          case 'site':
            scopeExists = await app.prisma.site.findFirst({ where: { id: request.body.scope_id, tenant_id: request.tenantId! } });
            break;
          case 'location':
            scopeExists = await app.prisma.location.findFirst({ where: { id: request.body.scope_id, tenant_id: request.tenantId! } });
            break;
          case 'department':
            scopeExists = await app.prisma.department.findFirst({ where: { id: request.body.scope_id, tenant_id: request.tenantId! } });
            break;
          case 'category':
            scopeExists = await app.prisma.category.findFirst({ where: { id: request.body.scope_id, tenant_id: request.tenantId! } });
            break;
        }
        if (!scopeExists) {
      ...
      if (request.body.auditor_ids?.length) {
        const auditors = await app.prisma.user.findMany({
          where: { id: { in: request.body.auditor_ids }, tenant_id: request.tenantId!, status: 'ACTIVE' },
        });
      ...
      // If start_at is now, set status to IN_PROGRESS
      ...
      // Notify auditors
      // await app.queues.notifications.add('audit_assigned', { sessionId: session.id, auditorIds: request.body.auditor_ids });
  ...
  api.post('/:id/start', {
    schema: {
      ...
      // Create items for all assets in scope
      let assets: any[] = [];
      switch (session.scope_type) {
        ...
        case 'category':
          ...
          break;
        case 'custom':
          // Would need a custom asset list stored in session
          break;
      }
      ...
      if (assets.length > 0) {
        await app.prisma.auditSessionItem.createMany({
          data: assets.map(a => ({
            ...
            asset_id: a.id,
            expected_location_id: null, // Would need to determine from asset
          }),
        });
      }
  ...
  api.post('/:id/scan', {
    schema: {
      ...
      response: {
        200: z.object({ message: z.string(), item: z.object({ id: z.string(), status: z.string() }) }),
        400: z.object({ error: z.string(), code: z.string() }),
        404: z.object({ error: z.string(), code: z.string() }),
      },
    }, async (request, reply) => {
  ...
  api.post('/:id/discrepancies/:discrepancyId/resolve', {
    schema: {
      ...
      if (status === 'RESOLVED') {
        await app.prisma.auditSession.update({
          where: { id: request.params.id },
          data: {
            // Would need to recalculate counts
          },
        });
      }
  ...
  api.get('/:id/report', {
    schema: {
      ...
      // PDF would require pdfkit or similar
