...
export async function agentRoutes(app: FastifyInstance) {
    ...
    schema: {
      ...
      response: {
        200: z.object({
          ...
          asset: z.object({ id: z.string(), asset_tag: z.string() }).nullable(),
          hardware: z.object({}).nullable(),
          software: z.array(z.object({})).nullable(),
        }),
  ...
  api.post('/', {
    schema: {
      ...
      response: {
        201: z.object({ id: z.string(), enrollment_token: z.string() }),
      },
    }, async (request, reply) => {
      const tenantId = request.tenantId!;
  ...
  // Regenerate token
  api.post('/:id/regenerate-token', {
    schema: {
      params: z.object({ id: z.string().uuid() }),
      response: {
        200: z.object({ enrollment_token: z.string() }),
        404: z.object({ error: z.string(), code: z.string() }),
      },
    }, async (request, reply) => {
      const enrollment = await app.prisma.agentEnrollment.findFirst({
  ...
  api.get('/:id/logs', {
    schema: {
      ...
      const where: any = { enrollment_id: request.params.id };
