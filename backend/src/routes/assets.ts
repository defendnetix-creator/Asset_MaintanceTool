...
import { z } from 'zod';
import { assetRoutes as assetRoutesUtil } from '../utils/asset-helpers';
...
export async function assetRoutes(app: FastifyInstance) {
    ...
    schema: {
      ...
      const asset = await app.prisma.asset.findFirst({
        ...
        include: {
          ...
          custom_fields: {
            include: {
              custom_field: { select: { id: true, name: true, label: true, type: true } },
            },
          },
  ...
  api.post('/', {
    schema: {
      ...
      response: {
        201: z.object({ id: z.string(), asset_tag: z.string() }),
      ...
      await app.prisma.assetEvent.create({
        data: {
          asset_id: asset.id,
          tenant_id: tenantId,
          event_type: 'CHECK_IN', // Initial creation
    ...
    schema: {
      ...
      if (request.body.custom_fields) {
        ...
        for (const [key, value] of Object.entries(request.body.custom_fields)) {
          const cf = await app.prisma.customField.findFirst({ where: { tenant_id: request.tenantId!, name: key, entity_type: 'asset', is_active: true } });
          if (cf) {
            const value: any = {};
            switch (cf.type) {
              case 'text': value.value_text = String(value); break;
    ...
    schema: {
      ...
      await app.prisma.assetEvent.create({
        data: {
          asset_id: asset.id,
          tenant_id: request.tenantId!,
          event_type: 'CHECK_OUT', // Using as delete event
          performed_by_id: request.user!.id,
  ...
  api.post('/bulk', {
    schema: {
      ...
      response: {
        200: z.object({ processed: z.number(), failed: z.number(), errors: z.array(z.object({ id: z.string(), error: z.string() })) }),
      },
    }, async (request, reply) => {
  ...
  api.post('/import/preview', {
    schema: {
      body: z.object({
        file: z.string(), // base64 encoded CSV/JSON
      ...
      response: {
        200: z.object({
          ...
          summary: z.object({ total: z.number(), valid: z.number(), invalid: z.number() }),
        }),
      },
    }, async (request, reply) => {
      // Implementation would parse CSV/JSON and validate each row
  ...
  api.post('/import/commit', {
    schema: {
      ...
      response: {
        200: z.object({ created: z.number(), errors: z.array(z.object({ row: z.number(), error: z.string() })) }),
      },
    }, async (request, reply) => {
      // Implementation would process validated rows and create assets
      // This is a placeholder for the actual implementation
